...
| 底部 ⚠ 字樣 | 當出現錯誤（如限流 429、Token 失效需重新登入、資料格式異常等）時的異常提示；限流時附「約 N 分後重試」倒數 |
...
- **自動重新整理**：成功後最多每 5 分鐘更新一次（可由右鍵「更新頻率」調整；以 30 秒心跳檢查；非限流的失敗會快速重試，但任何重試至少間隔 20 秒，429 則一律等退避結束）。同一時間只允許一個背景請求在途，避免並發競爭。
...
| 顯示「限流(429)」 | 官方 API 端點暫時限制呼叫。小工具會自動進入冷卻退避（120 秒起、每次加倍、上限 15 分鐘），期間不再呼叫，稍候自行重試，畫面會顯示「約 N 分後重試」倒數（v1.5.2 起）；**剛啟動尚無資料時也一樣守冷卻**（v1.5.1 修正：舊版此時會每 30 秒重打，把限流越養越久）。重度使用 Claude Code 時較易遇到，屬正常現象。 |
